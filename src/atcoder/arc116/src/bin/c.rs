...
pub fn main() {
    ...
    let m = sc.usize();
    const MAX: usize = 20;
    let mut dp = vec![vec![Mod::new(0); m + 1]; MAX];
    for first_val in 1..=m {
        dp[1][first_val] = Mod::new(1);
    }
    for cnt_diff in 1..MAX {
        for last in 1..=m {
            if dp[cnt_diff][last] == Mod::new(0) {
                continue;
            }
            for next in (last + last..=m).step_by(last) {
                dp[cnt_diff + 1][next] = dp[cnt_diff + 1][next] + dp[cnt_diff][last];
            }
        }
    }
    let mut c = vec![vec![Mod::new(0); MAX + 1]; n + MAX + 2];
    c[0][0] = Mod::new(1);
    for i in 1..c.len() {
        c[i][0] = Mod::new(1);
        for j in 1..c[i].len() {
    ...
    for cnt_diff in 1..MAX {
        for last in 1..=m {
            ...
            let more = n - cnt_diff;
            res = res + c[more + cnt_diff - 1][cnt_diff - 1] * dp[cnt_diff][last];
        }
    }
