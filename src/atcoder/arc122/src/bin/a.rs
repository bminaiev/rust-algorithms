...
pub fn main() {
    ...
    let n = sc.usize();
    let a = sc.vec::<i32>(n);
    let mut dp_minus = vec![Mod::ZERO; n + 1];
    let mut dp_plus = vec![Mod::ZERO; n + 1];
    ...
    for i in 1..=n {
        ...
        dp[i] = dp_plus[i] + dp_minus[i];
    }
    let mut res = dp[n - 1] * Mod::new(a[0]);
    for i in 1..n {
        let left = i - 1;
        let right = n - i - 1;
        let ways_plus = dp[left] * dp[right];
        res += ways_plus * Mod::new(a[i]);
        let i1 = max(0, left as i32 - 1) as usize;
        let i2 = max(0, right as i32 - 1) as usize;
        let ways_minus = dp[i1] * dp[i2];
        res -= ways_minus * Mod::new(a[i]);
    }
