...
pub fn main() {
    ...
    for _ in 0..tc {
        let n = sc.usize();
        let k = sc.usize() - 1;
        let mut dp_right = vec![vec![Mod::new(0); k + 1]; n + 1];
        let mut dp_left = vec![vec![Mod::new(0); k + 1]; n + 1];
        for layer in 1..=k {
            for pos in (0..n).rev() {
                dp_right[pos][layer] = dp_right[pos + 1][layer] + Mod::new(1);
                if pos > 0 {
                    dp_right[pos][layer] += dp_left[pos - 1][layer - 1];
                }
            }
            for pos in 0..n {
                dp_left[pos][layer] = dp_right[pos + 1][layer - 1] + Mod::new(1);
                if pos > 0 {
                    let tmp = dp_left[pos - 1][layer];
                    dp_left[pos][layer] += tmp;
                }
            }
        }
        let res = dp_right[0][k] + Mod::new(1);
