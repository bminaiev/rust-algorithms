...
pub fn main() {
    ...
    let n = sc.usize();
    let c = sc.vec::<usize>(n);
    let b = sc.vec::<usize>(n - 1);

    let q = sc.usize();
    for _ in 0..q {
        let smallest = sc.i64();
        let mut ways = vec![Mod::ONE];
        let mut sum = 0;
        let mut sum_b = 0;
        for i in 0..n {
            let mut next_ways = vec![Mod::ZERO; ways.len() + c[i]];
            for old in 0..ways.len() {
                for add in 0..=c[i] {
                    next_ways[old + add] += ways[old];
                }
            }
            ways = next_ways;
