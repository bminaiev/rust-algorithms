...
pub fn main() {
    ...
    for x in 1..=n {
        write!(out, "{} ", rnd.next_in_range(0, x)).unwrap();
    }
    writeln!(out).unwrap();
    let q = 100_000;
    writeln!(out, "{}", q).unwrap();
    for _ in 0..q {
        let q_type = rnd.next_in_range(1, 2);
        if q_type == 0 {
            let pos = rnd.next_in_range(0, n);
            let val = rnd.next_in_range(0, pos + 1);
            writeln!(out, "1 {} {}", pos + 1, val).unwrap();
        } else {
            assert_eq!(q_type, 1);
            let pos = rnd.next_in_range(0, n);
            writeln!(out, "2 {}", pos + 1).unwrap();
        }
    }
}
