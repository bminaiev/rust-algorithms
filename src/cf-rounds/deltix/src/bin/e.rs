...
pub fn main() {
    ...
    let mut fact = vec![Mod::ONE; MAX];
    let mut fact_inv = vec![Mod::ONE; MAX];
    for i in 1..MAX {
        fact[i] = fact[i - 1] * Mod::new(i as i32);
    }
    fact_inv[MAX - 1] = fact[MAX - 1].inv();
    for i in (1..(MAX - 1)).rev() {
        fact_inv[i] = fact_inv[i + 1] * Mod::new((i + 1) as i32);
    }
    let c = |n: usize, k: usize| -> Mod {
    ...
    for _ in 0..tc {
        ...
        let mut res = Mod::ONE;
        for cnt in 1..n {
            let total_ways = c(n, cnt);
            let add = (cnt - 1) * (k - 1);
            if add + cnt > n {
                break;
            }
            let cur_ways = c(n - add, cnt);
            res += cur_ways / total_ways;
        }
