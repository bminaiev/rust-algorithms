...
fn solve_one_test(sc: &mut Scanner, out: &mut BufWriter<File>, test_n: usize) {
    ...
    let b = sc.usize();
    let smallest = n + m - 1;
    let possible = smallest <= a && smallest <= b;
    let ans = if possible { "Possible" } else { "Impossible" };
    writeln!(out, "Case #{}: {}", test_n, ans).unwrap();
    if possible {
        let right_corner = a - smallest + 1;
        let left_corner = b - smallest + 1;
        for i in 0..n {
            for j in 0..m {
                let value = if i == n - 1 {
                    if j == 0 { left_corner } else if j == m - 1 { right_corner } else { 1 }
                } else {
                    1
                };
