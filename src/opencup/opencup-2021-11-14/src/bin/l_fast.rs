...
fn solve(
    ...
    let upd_max = |what: &mut i32, with: i32| {
        *what = max(*what, with);
    };
    ...
    for x in 0..=n {
        for y in 1..=m {
            ...
            if x < n {
                ...
                upd_max(&mut next_x.d00, cur.d00);
                upd_max(&mut next_x.d10, cur.d00 - install_first[x]);

                upd_max(&mut next_x.d00, cur.d10);
                upd_max(&mut next_x.d10, cur.d10 - install_first[x]);

                upd_max(&mut next_x.d01, max(cur.d01, cur.d11));
                upd_max(
                    &mut next_x.d11,
                    max(cur.d01, cur.d11) - install_first[x] + w[x][y - 1],
                );
            ...
            if y < m {
                ...
                upd_max(&mut next_y.d00, cur.d00);
                upd_max(&mut next_y.d01, cur.d00 - install_second[y]);

                upd_max(&mut next_y.d00, cur.d01);
                upd_max(&mut next_y.d01, cur.d01 - install_second[y]);

                upd_max(&mut next_y.d10, max(cur.d10, cur.d11));
                if x > 0 {
                    upd_max(
                        &mut next_y.d11,
                        max(cur.d10, cur.d11) - install_second[y] + w[x - 1][y],
                    );
                }
                dp[x][y + 1] = next_y;
            }
        ...
        for mid2 in l2..r2 {
            for use_mid in 0..2 {
                ...
                for x in 0..size_first {
                    let w_one_line = &w[mid1 + x];
                    for (dst, src) in state.part_w[x][0..size_first]
                        .iter_mut()
                        .zip(w_one_line[mid2..].iter())
                    {
                        *dst = *src;
                    }
                }
                ...
                for x in 0..size_first {
                    state.part_install_first[x] = install_first[x + mid1];
                }
                ...
                for y in 0..size_second {
                    state.part_install_second[y] = install_second[y + mid2];
                }
                ...
                for x in 0..size_first {
                    let w_one_line = &w[start_first - x];
                    for (dst, src) in state.part_w[x][0..size_second].iter_mut().zip(
                        w_one_line[start_second + 1 - size_second..start_second]
                            .iter()
                            .rev(),
                    ) {
                        *dst = *src;
