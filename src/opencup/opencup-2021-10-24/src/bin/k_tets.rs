...
fn gen_perm(n: usize, rnd: &mut Random) -> Vec<usize> {
    let mut r = vec![0; n];
    for i in 0..n {
        r[i] = i;
        let idx = rnd.next_in_range(0, i + 1);
        r.swap(i, idx);
    }
    r
}
...
pub fn main() {
    ...
    let n = 200_000;//sc.usize();
    let mut rnd = Random::new(787788);

    let perm1 = gen_perm(n, &mut rnd);
    let perm2 = gen_perm(n, &mut rnd);
    let perm3 = gen_perm(n, &mut rnd);
    ...
    let tc = 200_000;//sc.usize();
    for _ in 0..tc {
        let from = rnd.next_in_range(0, n) + 1;//sc.usize();
        let to = rnd.next_in_range(0, n) + 1;//sc.usize();
